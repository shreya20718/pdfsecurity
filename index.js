...
app.post("/send-eye-report", upload.single('report'), async (req, res) => {
  ...
  try {
    ...
    const emailResult = await transporter.sendMail({
      ...
      subject: `👁 Optimate Eye Measurement Report - ${new Date().toLocaleDateString()}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #ff6b35;">👁 Optimate Eye Measurement Report</h2>
        ...
        <div class="footer">
            ...
            async function sendToOwner() {
                ...
                if (!measurementData.hasData) {
                    showToast('⚠ Please capture eye photo first!', 'error');
                    return;
                }
